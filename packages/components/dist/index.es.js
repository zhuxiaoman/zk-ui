import { createBlock as f, openBlock as g, unref as h, mergeProps as k, withCtx as _, renderSlot as P } from "vue";
...
}, u = "zk-ui-theme", y = "zk-ui-primary";
function M(t, e, r) {
  return Math.min(r, Math.max(e, t));
}
function b(t) {
  const e = t.replace("#", ""), r = parseInt(
    e.length === 3 ? e.split("").map((o) => o + o).join("") : e,
    // 如果已经是6位格式，直接使用
    16
    // 指定为16进制解析
  );
...
function p(t) {
  if (!t) return;
  const e = t.startsWith("#") ? t : `#${t}`, r = B(e);
  Z(r);
  try {
...
function T() {
  const e = (localStorage.getItem(u) || (document.documentElement.getAttribute("data-theme") === "dark" ? "dark" : "light")) === "dark" ? "light" : "dark";
  return c(e), e;
}
function d() {
  try {
    const t = localStorage.getItem(u), e = localStorage.getItem(y);
    t && c(t), e && p(e);
  } catch {
  }
}
function L() {
  return localStorage.getItem(y) || getComputedStyle(document.documentElement).getPropertyValue("--el-color-primary").trim() || "#409EFF";
}
